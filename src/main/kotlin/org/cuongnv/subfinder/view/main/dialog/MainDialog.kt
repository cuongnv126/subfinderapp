...
import org.cuongnv.subfinder.view.main.MainMvpView
import org.cuongnv.subfinder.view.main.MainPresenter
...
class MainDialog(owner: Frame?) : Dialog(owner, 480, 450), MainMvpView {
    ...
    override fun onCreateView() {
        ...
        val title = TextView("sub finder").apply {
    ...
    private fun createFilesTable(): JScrollPane {
        ...
        val header = tableFiles.tableHeader.apply {
            preferredSize = Dimension(tableFiles.width, 30)
    ...
    private fun searchSub() {
        val keyword = edtQuery.text
        if (keyword.isNotEmpty()) {
            progressView.startProgress()
            MainPresenter.getInstance().search(keyword)
        }
    }
    ...
    override fun onSearchSuccess(data: List<SimpleSubtitle>) {
        ...
        for (row in data) {
            model.addRow(arrayOf(row.name, row.owner))
        }
    }
    ...
    override fun onClose() {
        MainPresenter.getInstance().dispose()
        runCatching { FileOutputStream(File("config.ini")).bufferedWriter().use { it.write(txtPath.text) } }
        super.onClose()
    }
    ...
    override fun show(x: Int, y: Int) {
        pack()
        super.show(x, y)
        TimedThread.getInstance().submitDelay({
            SwingUtilities.invokeLater {
                edtQuery.requestFocusInWindow()
            }
        }, 50)
    }
}